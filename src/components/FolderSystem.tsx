...
export function FolderSystem() {
  ...
  const { folders, recordings, addFolder, deleteFolder } = useRecordings();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {folders.map((folder) => {
          const folderRecordings = recordings.filter(
            (recording) => recording.folderId === folder.id
          );
          
          const folderDate = folder.createdAt 
            ? format(parseISO(folder.createdAt), "d MMM yyyy", { locale: es })
            : "";
          
          return (
            <div
              key={folder.id}
              className="group bg-card hover:bg-accent rounded-lg p-4 transition-colors cursor-pointer flex flex-col justify-between h-32"
      ...
      <Dialog open={showNewFolderDialog} onOpenChange={setShowNewFolderDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button 
              onClick={() => handleCreateFolder(newFolderName, folderColor)}
