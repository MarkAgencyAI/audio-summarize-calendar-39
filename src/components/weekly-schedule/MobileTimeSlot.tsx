...
export function MobileTimeSlot({ event, onClick, onDelete, getFolderName }: MobileTimeSlotProps) {
  if (!event) {
    return (
      <div 
        className="h-14 border-l border-b border-border p-0.5 cursor-pointer transition-colors hover:bg-accent/20 relative min-w-[40px] flex items-center justify-center"
        onClick={onClick}
      >
