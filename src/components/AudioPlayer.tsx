...
export const AudioPlayer = forwardRef<AudioPlayerHandle, AudioPlayerProps>(({
  ...
  return <div className="w-[95%] sm:w-[95%] bg-background border rounded-md p-3 shadow-sm overflow-hidden">
      <div className="space-y-3 max-w-full">
        ...
        <div ref={waveformRef} className="relative w-full h-16 bg-gray-100 rounded-lg overflow-hidden cursor-pointer" onClick={handleWaveformClick} onMouseDown={handleWaveformMouseDown} onMouseMove={handleWaveformMouseMove} onMouseUp={handleWaveformMouseUp} onMouseLeave={handleWaveformMouseUp}>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <svg viewBox="0 0 100 50" preserveAspectRatio="none" className="w-full h-full">
              <path d="M 0,25 Q 10,20 20,25 T 40,25 T 60,25 T 80,25 T 100,25" stroke="rgba(209, 213, 219, 0.8)" strokeWidth="1" fill="none" />
              <path d="M 0,25 Q 10,30 20,25 T 40,25 T 60,25 T 80,25 T 100,25" stroke="rgba(209, 213, 219, 0.8)" strokeWidth="1" fill="none" />
            </svg>
          </div>
          
          {/* Played portion of waveform */}
          <div className="absolute top-0 bottom-0 left-0 flex items-center overflow-hidden" style={{
          width: `${currentTime / validDuration * 100}%`
        }}>
            <svg viewBox="0 0 100 50" preserveAspectRatio="none" className="w-full h-full">
              <path d="M 0,25 Q 10,20 20,25 T 40,25 T 60,25 T 80,25 T 100,25" stroke="var(--color-primary, #0085FF)" strokeWidth="1.5" fill="none" />
              <path d="M 0,25 Q 10,30 20,25 T 40,25 T 60,25 T 80,25 T 100,25" stroke="var(--color-primary, #0085FF)" strokeWidth="1.5" fill="none" />
            </svg>
          </div>
          
          {/* Selected region */}
          {selectionLeftPercent !== null && selectionWidthPercent !== null && <div className="absolute h-full bg-blue-500/30 border-l border-r border-blue-500 pointer-events-none" style={{
        ...
        <div className="w-full space-y-1">
          <Slider value={[currentTime]} min={0} max={validDuration} step={0.1} onValueChange={handleSeek} onValueCommit={() => setIsDragging(false)} onPointerDown={() => setIsDragging(true)} className="cursor-pointer" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 items-center gap-3">
          <div className="flex items-center justify-center sm:justify-start gap-2">
            <Button variant="ghost" size="icon" onClick={skipBackward} aria-label="Retroceder 10 segundos" className="h-8 w-8">
              <SkipBack className="h-4 w-4" />
            </Button>
            
            <Button variant="ghost" size="icon" onClick={togglePlayPause} className="h-9 w-9" disabled={isLoading} aria-label={isPlaying ? "Pausar" : "Reproducir"}>
              {isLoading ? <div className="h-4 w-4 border-2 border-primary border-t-transparent rounded-full animate-spin" /> : isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
            </Button>
            
            <Button variant="ghost" size="icon" onClick={skipForward} aria-label="Avanzar 10 segundos" className="h-8 w-8">
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-2">
            {/* Selection controls */}
            {selectionStart !== null && selectionEnd !== null && <Button variant="outline" size="sm" onClick={handleAddChapter} className="h-7 text-xs flex items-center gap-1">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" onClick={() => changePlaybackRate(1)} className={`h-6 px-2 text-xs ${playbackRate === 1 ? 'bg-primary/20' : ''}`}>
                1x
              </Button>
              <Button variant="ghost" size="sm" onClick={() => changePlaybackRate(1.5)} className={`h-6 px-2 text-xs ${playbackRate === 1.5 ? 'bg-primary/20' : ''}`}>
                1.5x
              </Button>
              <Button variant="ghost" size="sm" onClick={() => changePlaybackRate(2)} className={`h-6 px-2 text-xs ${playbackRate === 2 ? 'bg-primary/20' : ''}`}>
                2x
              </Button>
            </div>
            ...
            {/* Volume control */}
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" onClick={toggleMute} className="h-7 w-7" aria-label={isMuted ? "Activar sonido" : "Silenciar"}>
                {isMuted ? <VolumeX className="h-3.5 w-3.5" /> : <Volume2 className="h-3.5 w-3.5" />}
              </Button>
              
              <Slider value={[isMuted ? 0 : volume]} min={0} max={1} step={0.01} onValueChange={handleVolumeChange} className="w-[60px]" aria-label="Volumen" />
            </div>
          </div>
        </div>
      </div>
