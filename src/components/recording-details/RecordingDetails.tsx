...
export function RecordingDetails({
  ...
  const renderPlayerContent = () => <div className="p-4 flex flex-col h-full">
      <h3 className="text-lg font-semibold mb-4">Reproductor de Audio</h3>
      <AudioPlayerV2 audioUrl={recording.audioUrl} audioBlob={audioBlob || undefined} initialDuration={recording.duration} onTimeUpdate={handleTimeUpdate} ref={audioPlayerRef} onDurationChange={setAudioDuration} onAddChapter={handleAddChapterFromPlayer} />
      
      {chapters && chapters.length > 0 && <div className="mt-3 max-w-full overflow-hidden">
  ...
  return <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <DialogContent className="p-0 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-lg w-[95vw] md:w-[90vw] lg:w-[80vw] max-w-6xl h-[90vh] flex flex-col overflow-hidden">
        <div className="flex flex-col w-full h-full">
          ...
          <Tabs value={mainView} onValueChange={value => setMainView(value as "player" | "content" | "notes")} className="flex-grow overflow-hidden flex flex-col">
