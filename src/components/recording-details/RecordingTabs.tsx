...
export function RecordingTabs({
  ...
  // Debug to verify if data is arriving correctly
  useEffect(() => {
    console.log("RecordingTabs data:", {
      activeTab: data.activeTab,
      chapters: chapters.length,
      hasOutput: hasTranscription,
      outputLength: data.recording.output?.length || 0,
      hasWebhookData: hasSummaryData
    });
  }, [data, chapters, hasSummaryData, hasTranscription]);
    ...
    >
      <div className="px-4 pt-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
        <TabsList className="grid grid-cols-3 gap-1 w-full bg-slate-100 dark:bg-slate-800/50 p-1 rounded-lg">
          <TabsTrigger 
            value="webhook" 
            className="flex items-center gap-1.5 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-900 data-[state=active]:shadow-sm rounded-md"
          >
